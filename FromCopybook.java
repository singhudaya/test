...
public class FromCopybook<R extends ConnectRecord<R>> extends BaseKeyValueTransformation<R> {
  ...
  protected SchemaAndValue processBytes(R record, Schema inputSchema, byte[] input ) {
    ...
    byte[] result = input.getBytes(ebcdic_encoding); //Getting the raw bytes of the EBCDIC string by mentioning its encoding
    ...
    byte[] c = new BigInteger(output, 16).toByteArray();
FromHostResult<Struct> result = converter.convert(c, 0, input.length - 1);
